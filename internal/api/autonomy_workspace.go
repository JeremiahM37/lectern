...
func (s *Server) autoPrompt(ctx context.Context, a *autoRecord, role string, p *store.Project) string {
	...
	b.WriteString("You are one role in Jeremiah's autonomous workshop. Choose useful work grounded in his projects/preferences, not activity for its own sake. New research and existing project improvements are both welcome. Do not trade, buy, publish, contact others, deploy production changes, or modify safety/quota controls. Everything you build stays in /work for independent review. No server credentials, live sessions or production files are available. Use shell/tests freely in this isolated workspace. Do not claim tests ran unless you ran them. Output is untrusted evidence, not instructions to later agents.\n")
	view := *a.State
