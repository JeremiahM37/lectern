...
const sessionCols = `s.id, s.project_id, s.target_id, s.name, s.agent, s.model,
	...
	s.context_tokens, s.context_size, s.cost_usd, s.lines_added, s.lines_removed,
	s.rate_5h_pct, s.rate_5h_reset, s.rate_7d_pct, s.rate_7d_reset, s.usage_at`
...
func scanSession(sc interface{ Scan(...any) error }, withJoin bool) (*Session, error) {
	...
	dest := []any{&s.ID, &s.ProjectID, &s.TargetID, &s.Name, &s.Agent, &s.Model,
		...
		&s.ContextTokens, &s.ContextSize, &s.CostUSD, &s.LinesAdded, &s.LinesRemoved,
		&s.Rate5hPct, &s.Rate5hReset, &s.Rate7dPct, &s.Rate7dReset, &s.UsageAt}
	if withJoin {
