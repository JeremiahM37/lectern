...
function NewTerminal({machines,projects,onNew}:{machines:TerminalMachine[];projects:TerminalProject[];onNew:(choice?:NewTerminalChoice)=>Promise<void>}){
 ...
 return <div className="terminal-new"><button className="terminal-new-main" disabled={busy} aria-label="New terminal" title="Open a blank shell on the default machine" onClick={()=>start()}>{busy?'Opening…':<>＋<span className="terminal-new-label"> New terminal</span></>}</button>{caret&&<details className="terminal-new-machines" ref={menu} onToggle={()=>{if(menu.current?.open){place();if(matchMedia("(pointer:fine)").matches)search.current?.focus({preventScroll:true});}else setQuery("");}}><summary onClick={event=>{event.preventDefault();if(menu.current){menu.current.open=!menu.current.open;place();}}} aria-label="New terminal in a project or on another machine" title="New terminal in a project or on another machine">▾</summary><div className="terminal-actions-panel terminal-new-picker" role="menu"><input ref={search} className="terminal-new-search" type="search" value={query} aria-label="Search projects and machines" placeholder="Search projects and machines" onChange={event=>setQuery(event.target.value)}/>{shownProjects.length>0&&<div className="terminal-new-group" role="presentation">Projects</div>}{shownProjects.map(project=><button key={'project-'+project.id} role="menuitem" className="terminal-new-project" disabled={busy} onClick={()=>start({projectID:project.id})}><span className="terminal-new-project-name">{project.name}</span><span className="terminal-new-project-path">{project.repo_path}{project.target_name?' · '+project.target_name:''}</span></button>)}{shownMachines.length>0&&<div className="terminal-new-group" role="presentation">Machines</div>}{shownMachines.map(machine=><button key={'machine-'+machine.id} role="menuitem" disabled={busy} onClick={()=>start({machineID:machine.id})}>{machine.name}</button>)}{shownMachines.length===0&&shownProjects.length===0&&<div className="terminal-new-empty" role="presentation">No matching projects or machines</div>}</div></details>}</div>;
}
